...
if len(T1N_file_List) != 1:
    raise ValueError(
        f"Expected exactly one T1N file, but found {len(T1N_file_List)} !"
    )
...
if len(T2W_file_List) != 1:
    raise ValueError(
        f"Expected exactly one T2W file, but found {len(T2W_file_List)} !"
    )
T2F_file_List = T2F_NII_DIR.files("*.nii.gz")
if len(T2F_file_List) != 1:
    raise ValueError(
        f"Expected exactly one T2F file, but found {len(T2F_file_List)} !"
    )
...
# TODO do preprocessing and segmentation magic
